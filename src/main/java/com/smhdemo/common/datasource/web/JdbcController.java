 ...
 */
@Controller
@RequestMapping(value = "/common/datasource/jdbc")
public class JdbcController extends CrudBaseController<Jdbc,Long>{
	@Autowired
	private DataSourceFac dataSourceFac;

	@Override
	protected Jdbc getVO(Long pk, Model model) {
		if(pk != null){
			try {
				return (Jdbc)dataSourceFac.getBase(pk);
			} catch (Exception e) {
			}			
		}
		return new Jdbc();
	...
	@Override
	protected Long addSaveOperator(Jdbc vo, BindingResult result, Model model)
			throws Exception {
		return dataSourceFac.addBase(vo);
	}

	@Override
	protected Long updSaveOperator(Jdbc vo, BindingResult result, Model model)
			throws Exception {
	...
	protected String queryOperator(List<QueryParameter> qps,
		...
		return Jdbc.class.getSimpleName();
	}

	@Override
	protected String getPagePath() {
		return "common/datasource/jdbc";
	}
