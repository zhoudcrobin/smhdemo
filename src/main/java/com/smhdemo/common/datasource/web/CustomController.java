 ...
 */
@Controller
@RequestMapping(value = "/common/datasource/custom")
public class CustomController extends CrudBaseController<Custom,Long>{
	@Autowired
	private DataSourceFac dataSourceFac;

	@Override
	protected Custom getVO(Long pk, Model model) {
		if(pk != null){
			try {
				return (Custom)dataSourceFac.getBase(pk);
			} catch (Exception e) {
			}			
		}
		return new Custom();
	...
	@Override
	protected Long addSaveOperator(Custom vo, BindingResult result, Model model)
			throws Exception {
		return dataSourceFac.addBase(vo);
	}

	@Override
	protected Long updSaveOperator(Custom vo, BindingResult result, Model model)
			throws Exception {
	...
	protected String getPagePath() {
		return "common/datasource/custom";
	}
